...
class VtkModel(QWidget):
    ...
    def _new_experimental_marker_link(self, virtual_to_experimental_markers_indices):
        ...
        for i, j in enumerate(virtual_to_experimental_markers_indices):
            if j is None:
                continue
            points = vtkPoints()
            points.InsertNextPoint(np.array(self.markers["model"].data[:3, j, 0].data).reshape(3, ))
            points.InsertNextPoint(np.array(self.markers["experimental"].data[:3, i, 0].data).reshape(3, ))
    ...
    def new_imus_set(self, all_imus):
        ...
        for i, imu in enumerate(all_imus):
            ...
            pts = vtkPoints()
            pts.InsertNextPoint(np.array([0, 0, 0]).reshape(3, ))
            pts.InsertNextPoint(np.array([1, 0, 0]).reshape(3, ))
            pts.InsertNextPoint(np.array([0, 1, 0]).reshape(3, ))
            pts.InsertNextPoint(np.array([0, 0, 1]).reshape(3, ))
        ...
        for i, imu in enumerate(all_imus):
            points = vtkPoints()
            points.InsertNextPoint(np.array([-1, -1, -1]).reshape(3, ))
            points.InsertNextPoint(np.array([-1, 1, -1]).reshape(3, ))
            points.InsertNextPoint(np.array([-1, -1, 1]).reshape(3, ))
            points.InsertNextPoint(np.array([-1, 1, 1]).reshape(3, ))
            points.InsertNextPoint(np.array([1, -1, -1]).reshape(3, ))
    ...
    def update_imus(self, all_imus):
        ...
        for i, imu in enumerate(self.all_imus):
            ...
            pts = vtkPoints()
            pts.InsertNextPoint(np.array(imu.meca.translation).reshape(3, ))
            pts.InsertNextPoint(np.array(imu.meca.translation + imu.isel(col=0)[0:3] * self.imus_length).reshape(3, ))
            pts.InsertNextPoint(np.array(imu.meca.translation + imu.isel(col=1)[0:3] * self.imus_length).reshape(3, ))
            pts.InsertNextPoint(np.array(imu.meca.translation + imu.isel(col=2)[0:3] * self.imus_length).reshape(3, ))
            ...
            points.InsertNextPoint(np.array(rototrans[:3, 3] + (rototrans[:3, :3] @ [-1, -1, -1]) * self.imus_length / 3).reshape(3, ))
            points.InsertNextPoint(np.array(rototrans[:3, 3] + (rototrans[:3, :3] @ [-1, 1, -1]) * self.imus_length / 3).reshape(3, ))
            points.InsertNextPoint(np.array(rototrans[:3, 3] + (rototrans[:3, :3] @ [-1, -1, 1]) * self.imus_length / 3).reshape(3, ))
            points.InsertNextPoint(np.array(rototrans[:3, 3] + (rototrans[:3, :3] @ [-1, 1, 1]) * self.imus_length / 3).reshape(3, ))
            points.InsertNextPoint(np.array(rototrans[:3, 3] + (rototrans[:3, :3] @ [1, -1, -1]) * self.imus_length / 3).reshape(3, ))
    ...
    def update_wrapping(self, all_wrappings):
        ...
        for seg, wrappings in enumerate(all_wrappings):
            ...
            for i, wrapping in enumerate(self.all_wrappings[seg]):
                ...
                for j in range(n_vertex):
                    points.InsertNextPoint(np.array(wrapping[0:3, j]).reshape(3, ))
